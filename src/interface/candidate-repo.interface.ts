...
export interface ICandidateRepo {
    // Candidate Item 생성
    // 모든 값을 받아야 함
    create(input: CandidateItemAttributes): Promise<CandidateItemAttributes>;
    ...
    deleteCandidateItem(candidateId: Pick<CandidateItemAttributes, 'id'>): Promise<number>;

    // candidateId 배열로 여러 Candidate Item 삭제
    deleteCandidatesByIds(
        candidateIds: Pick<CandidateItemAttributes, 'id'>[] | string[]
    ): Promise<number>;
}
